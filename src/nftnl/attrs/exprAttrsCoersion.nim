...
macro attrOp*(
    e: typed, attr: static[int], exprKind: static[string], args: varargs[untyped]
): untyped =
  if args.len == 0:
    result = quote:
      rawGetAttr[expectedType(`attr`, `exprKind`)](`e`.raw, `attr`.uint16)
  elif args.len == 1:
    let arg0 = args[0]
    result = quote:
      rawSetAttr(`e`.raw, `attr`.uint16, `arg0`)
  else:
    error("attrOp takes 0 or 1 arguments")

# --- convenience wrappers -----------------------------------------------------

template setAttrCmp*(e: CmpExpr, attr: static[int], val: untyped): untyped =
  attrOp(e, attr, "cmp", val)

template getAttrCmp*(e: CmpExpr, attr: static[int]): untyped =
  attrOp(e, attr, "cmp")
