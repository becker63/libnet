...
proc rawGetAttr*[T](e: ptr struct_nftnl_expr, attr: uint16): T =
  when T is string:
    ...
    var size: uint32
    let p = nftnl_expr_get(e, attr, addr size) # returns pointer to data
    result = @[]
...
macro attrOp*(
    e: ExprConcept, exprKind: static[string], attr: untyped, args: varargs[untyped]
): untyped =
...
template setAttrCmp*(e: ExprConcept, attr: int, val: untyped): untyped =
  attrOp(e, "cmp", attr, val)

template getAttrCmp*(e: ExprConcept, attr: int): untyped =
  attrOp(e, "cmp", attr)
