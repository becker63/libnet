// Generated by the protocol buffer compiler.  DO NOT EDIT!
// NO CHECKED-IN PROTOBUF GENCODE
// source: test.proto
// Protobuf C++ Version: 6.32.1

#include "test.pb.h"

#include <algorithm>
#include <type_traits>
#include "google/protobuf/io/coded_stream.h"
#include "google/protobuf/generated_message_tctable_impl.h"
#include "google/protobuf/extension_set.h"
#include "google/protobuf/generated_message_util.h"
#include "google/protobuf/wire_format_lite.h"
#include "google/protobuf/descriptor.h"
#include "google/protobuf/generated_message_reflection.h"
#include "google/protobuf/reflection_ops.h"
#include "google/protobuf/wire_format.h"
// @@protoc_insertion_point(includes)

// Must be included last.
#include "google/protobuf/port_def.inc"
PROTOBUF_PRAGMA_INIT_SEG
namespace _pb = ::google::protobuf;
namespace _pbi = ::google::protobuf::internal;
namespace _fl = ::google::protobuf::internal::field_layout;
namespace libfuzzer_example {

inline constexpr Msg::Impl_::Impl_(
    ::_pbi::ConstantInitialized) noexcept
      : _cached_size_{0},
        optional_string_(
            &::google::protobuf::internal::fixed_address_empty_string,
            ::_pbi::ConstantInitialized()),
        any_{nullptr},
        optional_uint64_{::uint64_t{0u}},
        optional_float_{0} {}

template <typename>
PROTOBUF_CONSTEXPR Msg::Msg(::_pbi::ConstantInitialized)
#if defined(PROTOBUF_CUSTOM_VTABLE)
    : ::google::protobuf::Message(Msg_class_data_.base()),
#else   // PROTOBUF_CUSTOM_VTABLE
    : ::google::protobuf::Message(),
#endif  // PROTOBUF_CUSTOM_VTABLE
      _impl_(::_pbi::ConstantInitialized()) {
}
struct MsgDefaultTypeInternal {
  PROTOBUF_CONSTEXPR MsgDefaultTypeInternal() : _instance(::_pbi::ConstantInitialized{}) {}
  ~MsgDefaultTypeInternal() {}
  union {
    Msg _instance;
  };
};

PROTOBUF_ATTRIBUTE_NO_DESTROY PROTOBUF_CONSTINIT
    PROTOBUF_ATTRIBUTE_INIT_PRIORITY1 MsgDefaultTypeInternal _Msg_default_instance_;
}  // namespace libfuzzer_example
static constexpr const ::_pb::EnumDescriptor *PROTOBUF_NONNULL *PROTOBUF_NULLABLE
    file_level_enum_descriptors_test_2eproto = nullptr;
static constexpr const ::_pb::ServiceDescriptor *PROTOBUF_NONNULL *PROTOBUF_NULLABLE
    file_level_service_descriptors_test_2eproto = nullptr;
const ::uint32_t
    TableStruct_test_2eproto::offsets[] ABSL_ATTRIBUTE_SECTION_VARIABLE(
        protodesc_cold) = {
        0x081, // bitmap
        PROTOBUF_FIELD_OFFSET(::libfuzzer_example::Msg, _impl_._has_bits_),
        7, // hasbit index offset
        PROTOBUF_FIELD_OFFSET(::libfuzzer_example::Msg, _impl_.optional_string_),
        PROTOBUF_FIELD_OFFSET(::libfuzzer_example::Msg, _impl_.optional_uint64_),
        PROTOBUF_FIELD_OFFSET(::libfuzzer_example::Msg, _impl_.optional_float_),
        PROTOBUF_FIELD_OFFSET(::libfuzzer_example::Msg, _impl_.any_),
        0,
        2,
        3,
        1,
};

static const ::_pbi::MigrationSchema
    schemas[] ABSL_ATTRIBUTE_SECTION_VARIABLE(protodesc_cold) = {
        {0, sizeof(::libfuzzer_example::Msg)},
};
static const ::_pb::Message* PROTOBUF_NONNULL const file_default_instances[] = {
    &::libfuzzer_example::_Msg_default_instance_._instance,
};
const char descriptor_table_protodef_test_2eproto[] ABSL_ATTRIBUTE_SECTION_VARIABLE(
    protodesc_cold) = {
    "\n\ntest.proto\022\021libfuzzer_example\032\031google/"
    "protobuf/any.proto\"\274\001\n\003Msg\022\034\n\017optional_s"
    "tring\030\001 \001(\tH\000\210\001\001\022\034\n\017optional_uint64\030\002 \001("
    "\004H\001\210\001\001\022\033\n\016optional_float\030\003 \001(\002H\002\210\001\001\022!\n\003a"
    "ny\030\004 \001(\0132\024.google.protobuf.AnyB\022\n\020_optio"
    "nal_stringB\022\n\020_optional_uint64B\021\n\017_optio"
    "nal_floatb\006proto3"
};
static const ::_pbi::DescriptorTable* PROTOBUF_NONNULL const
    descriptor_table_test_2eproto_deps[1] = {
        &::descriptor_table_google_2fprotobuf_2fany_2eproto,
};
static ::absl::once_flag descriptor_table_test_2eproto_once;
PROTOBUF_CONSTINIT const ::_pbi::DescriptorTable descriptor_table_test_2eproto = {
    false,
    false,
    257,
    descriptor_table_protodef_test_2eproto,
    "test.proto",
    &descriptor_table_test_2eproto_once,
    descriptor_table_test_2eproto_deps,
    1,
    1,
    schemas,
    file_default_instances,
    TableStruct_test_2eproto::offsets,
    file_level_enum_descriptors_test_2eproto,
    file_level_service_descriptors_test_2eproto,
};
namespace libfuzzer_example {
// ===================================================================

class Msg::_Internal {
 public:
  using HasBits =
      decltype(::std::declval<Msg>()._impl_._has_bits_);
  static constexpr ::int32_t kHasBitsOffset =
      8 * PROTOBUF_FIELD_OFFSET(Msg, _impl_._has_bits_);
};

void Msg::clear_any() {
  ::google::protobuf::internal::TSanWrite(&_impl_);
  if (_impl_.any_ != nullptr) _impl_.any_->Clear();
  _impl_._has_bits_[0] &= ~0x00000002U;
}
Msg::Msg(::google::protobuf::Arena* PROTOBUF_NULLABLE arena)
#if defined(PROTOBUF_CUSTOM_VTABLE)
    : ::google::protobuf::Message(arena, Msg_class_data_.base()) {
#else   // PROTOBUF_CUSTOM_VTABLE
    : ::google::protobuf::Message(arena) {
#endif  // PROTOBUF_CUSTOM_VTABLE
  SharedCtor(arena);
  // @@protoc_insertion_point(arena_constructor:libfuzzer_example.Msg)
}
PROTOBUF_NDEBUG_INLINE Msg::Impl_::Impl_(
    [[maybe_unused]] ::google::protobuf::internal::InternalVisibility visibility,
    [[maybe_unused]] ::google::protobuf::Arena* PROTOBUF_NULLABLE arena, const Impl_& from,
    [[maybe_unused]] const ::libfuzzer_example::Msg& from_msg)
      : _has_bits_{from._has_bits_},
        _cached_size_{0},
        optional_string_(arena, from.optional_string_) {}

Msg::Msg(
    ::google::protobuf::Arena* PROTOBUF_NULLABLE arena,
    const Msg& from)
#if defined(PROTOBUF_CUSTOM_VTABLE)
    : ::google::protobuf::Message(arena, Msg_class_data_.base()) {
#else   // PROTOBUF_CUSTOM_VTABLE
    : ::google::protobuf::Message(arena) {
#endif  // PROTOBUF_CUSTOM_VTABLE
  Msg* const _this = this;
  (void)_this;
  _internal_metadata_.MergeFrom<::google::protobuf::UnknownFieldSet>(
      from._internal_metadata_);
  new (&_impl_) Impl_(internal_visibility(), arena, from._impl_, from);
  ::uint32_t cached_has_bits = _impl_._has_bits_[0];
  _impl_.any_ = ((cached_has_bits & 0x00000002U) != 0)
                ? ::google::protobuf::Message::CopyConstruct(arena, *from._impl_.any_)
                : nullptr;
  ::memcpy(reinterpret_cast<char *>(&_impl_) +
               offsetof(Impl_, optional_uint64_),
           reinterpret_cast<const char *>(&from._impl_) +
               offsetof(Impl_, optional_uint64_),
           offsetof(Impl_, optional_float_) -
               offsetof(Impl_, optional_uint64_) +
               sizeof(Impl_::optional_float_));

  // @@protoc_insertion_point(copy_constructor:libfuzzer_example.Msg)
}
PROTOBUF_NDEBUG_INLINE Msg::Impl_::Impl_(
    [[maybe_unused]] ::google::protobuf::internal::InternalVisibility visibility,
    [[maybe_unused]] ::google::protobuf::Arena* PROTOBUF_NULLABLE arena)
      : _cached_size_{0},
        optional_string_(arena) {}

inline void Msg::SharedCtor(::_pb::Arena* PROTOBUF_NULLABLE arena) {
  new (&_impl_) Impl_(internal_visibility(), arena);
  ::memset(reinterpret_cast<char *>(&_impl_) +
               offsetof(Impl_, any_),
           0,
           offsetof(Impl_, optional_float_) -
               offsetof(Impl_, any_) +
               sizeof(Impl_::optional_float_));
}
Msg::~Msg() {
  // @@protoc_insertion_point(destructor:libfuzzer_example.Msg)
  SharedDtor(*this);
}
inline void Msg::SharedDtor(MessageLite& self) {
  Msg& this_ = static_cast<Msg&>(self);
  if constexpr (::_pbi::DebugHardenVerifyHasBitConsistency()) {
    this_.VerifyHasBitConsistency();
  }
  this_._internal_metadata_.Delete<::google::protobuf::UnknownFieldSet>();
  ABSL_DCHECK(this_.GetArena() == nullptr);
  this_._impl_.optional_string_.Destroy();
  delete this_._impl_.any_;
  this_._impl_.~Impl_();
}

inline void* PROTOBUF_NONNULL Msg::PlacementNew_(
    const void* PROTOBUF_NONNULL, void* PROTOBUF_NONNULL mem,
    ::google::protobuf::Arena* PROTOBUF_NULLABLE arena) {
  return ::new (mem) Msg(arena);
}
constexpr auto Msg::InternalNewImpl_() {
  return ::google::protobuf::internal::MessageCreator::CopyInit(sizeof(Msg),
                                            alignof(Msg));
}
constexpr auto Msg::InternalGenerateClassData_() {
  return ::google::protobuf::internal::ClassDataFull{
      ::google::protobuf::internal::ClassData{
          &_Msg_default_instance_._instance,
          &_table_.header,
          nullptr,  // OnDemandRegisterArenaDtor
          nullptr,  // IsInitialized
          &Msg::MergeImpl,
          ::google::protobuf::Message::GetNewImpl<Msg>(),
#if defined(PROTOBUF_CUSTOM_VTABLE)
          &Msg::SharedDtor,
          ::google::protobuf::Message::GetClearImpl<Msg>(), &Msg::ByteSizeLong,
              &Msg::_InternalSerialize,
#endif  // PROTOBUF_CUSTOM_VTABLE
          PROTOBUF_FIELD_OFFSET(Msg, _impl_._cached_size_),
          false,
      },
      &Msg::kDescriptorMethods,
      &descriptor_table_test_2eproto,
      nullptr,  // tracker
  };
}

PROTOBUF_CONSTINIT PROTOBUF_ATTRIBUTE_INIT_PRIORITY1 const
    ::google::protobuf::internal::ClassDataFull Msg_class_data_ =
        Msg::InternalGenerateClassData_();

PROTOBUF_ATTRIBUTE_WEAK const ::google::protobuf::internal::ClassData* PROTOBUF_NONNULL
Msg::GetClassData() const {
  ::google::protobuf::internal::PrefetchToLocalCache(&Msg_class_data_);
  ::google::protobuf::internal::PrefetchToLocalCache(Msg_class_data_.tc_table);
  return Msg_class_data_.base();
}
PROTOBUF_CONSTINIT PROTOBUF_ATTRIBUTE_INIT_PRIORITY1
const ::_pbi::TcParseTable<2, 4, 1, 45, 2>
Msg::_table_ = {
  {
    PROTOBUF_FIELD_OFFSET(Msg, _impl_._has_bits_),
    0, // no _extensions_
    4, 24,  // max_field_number, fast_idx_mask
    offsetof(decltype(_table_), field_lookup_table),
    4294967280,  // skipmap
    offsetof(decltype(_table_), field_entries),
    4,  // num_field_entries
    1,  // num_aux_entries
    offsetof(decltype(_table_), aux_entries),
    Msg_class_data_.base(),
    nullptr,  // post_loop_handler
    ::_pbi::TcParser::GenericFallback,  // fallback
    #ifdef PROTOBUF_PREFETCH_PARSE_TABLE
    ::_pbi::TcParser::GetTable<::libfuzzer_example::Msg>(),  // to_prefetch
    #endif  // PROTOBUF_PREFETCH_PARSE_TABLE
  }, {{
    // .google.protobuf.Any any = 4;
    {::_pbi::TcParser::FastMtS1,
     {34, 1, 0, PROTOBUF_FIELD_OFFSET(Msg, _impl_.any_)}},
    // optional string optional_string = 1;
    {::_pbi::TcParser::FastUS1,
     {10, 0, 0, PROTOBUF_FIELD_OFFSET(Msg, _impl_.optional_string_)}},
    // optional uint64 optional_uint64 = 2;
    {::_pbi::TcParser::SingularVarintNoZag1<::uint64_t, offsetof(Msg, _impl_.optional_uint64_), 2>(),
     {16, 2, 0, PROTOBUF_FIELD_OFFSET(Msg, _impl_.optional_uint64_)}},
    // optional float optional_float = 3;
    {::_pbi::TcParser::FastF32S1,
     {29, 3, 0, PROTOBUF_FIELD_OFFSET(Msg, _impl_.optional_float_)}},
  }}, {{
    65535, 65535
  }}, {{
    // optional string optional_string = 1;
    {PROTOBUF_FIELD_OFFSET(Msg, _impl_.optional_string_), _Internal::kHasBitsOffset + 0, 0, (0 | ::_fl::kFcOptional | ::_fl::kUtf8String | ::_fl::kRepAString)},
    // optional uint64 optional_uint64 = 2;
    {PROTOBUF_FIELD_OFFSET(Msg, _impl_.optional_uint64_), _Internal::kHasBitsOffset + 2, 0, (0 | ::_fl::kFcOptional | ::_fl::kUInt64)},
    // optional float optional_float = 3;
    {PROTOBUF_FIELD_OFFSET(Msg, _impl_.optional_float_), _Internal::kHasBitsOffset + 3, 0, (0 | ::_fl::kFcOptional | ::_fl::kFloat)},
    // .google.protobuf.Any any = 4;
    {PROTOBUF_FIELD_OFFSET(Msg, _impl_.any_), _Internal::kHasBitsOffset + 1, 0, (0 | ::_fl::kFcOptional | ::_fl::kMessage | ::_fl::kTvTable)},
  }},
  {{
      {::_pbi::TcParser::GetTable<::google::protobuf::Any>()},
  }},
  {{
    "\25\17\0\0\0\0\0\0"
    "libfuzzer_example.Msg"
    "optional_string"
  }},
};
PROTOBUF_NOINLINE void Msg::Clear() {
// @@protoc_insertion_point(message_clear_start:libfuzzer_example.Msg)
  ::google::protobuf::internal::TSanWrite(&_impl_);
  ::uint32_t cached_has_bits = 0;
  // Prevent compiler warnings about cached_has_bits being unused
  (void) cached_has_bits;

  cached_has_bits = _impl_._has_bits_[0];
  if ((cached_has_bits & 0x00000003U) != 0) {
    if ((cached_has_bits & 0x00000001U) != 0) {
      _impl_.optional_string_.ClearNonDefaultToEmpty();
    }
    if ((cached_has_bits & 0x00000002U) != 0) {
      ABSL_DCHECK(_impl_.any_ != nullptr);
      _impl_.any_->Clear();
    }
  }
  if ((cached_has_bits & 0x0000000cU) != 0) {
    ::memset(&_impl_.optional_uint64_, 0, static_cast<::size_t>(
        reinterpret_cast<char*>(&_impl_.optional_float_) -
        reinterpret_cast<char*>(&_impl_.optional_uint64_)) + sizeof(_impl_.optional_float_));
  }
  _impl_._has_bits_.Clear();
  _internal_metadata_.Clear<::google::protobuf::UnknownFieldSet>();
}

#if defined(PROTOBUF_CUSTOM_VTABLE)
::uint8_t* PROTOBUF_NONNULL Msg::_InternalSerialize(
    const ::google::protobuf::MessageLite& base, ::uint8_t* PROTOBUF_NONNULL target,
    ::google::protobuf::io::EpsCopyOutputStream* PROTOBUF_NONNULL stream) {
  const Msg& this_ = static_cast<const Msg&>(base);
#else   // PROTOBUF_CUSTOM_VTABLE
::uint8_t* PROTOBUF_NONNULL Msg::_InternalSerialize(
    ::uint8_t* PROTOBUF_NONNULL target,
    ::google::protobuf::io::EpsCopyOutputStream* PROTOBUF_NONNULL stream) const {
  const Msg& this_ = *this;
#endif  // PROTOBUF_CUSTOM_VTABLE
  if constexpr (::_pbi::DebugHardenVerifyHasBitConsistency()) {
    this_.VerifyHasBitConsistency();
  }
  // @@protoc_insertion_point(serialize_to_array_start:libfuzzer_example.Msg)
  ::uint32_t cached_has_bits = 0;
  (void)cached_has_bits;

  cached_has_bits = this_._impl_._has_bits_[0];
  // optional string optional_string = 1;
  if ((cached_has_bits & 0x00000001U) != 0) {
    const ::std::string& _s = this_._internal_optional_string();
    ::google::protobuf::internal::WireFormatLite::VerifyUtf8String(
        _s.data(), static_cast<int>(_s.length()), ::google::protobuf::internal::WireFormatLite::SERIALIZE, "libfuzzer_example.Msg.optional_string");
    target = stream->WriteStringMaybeAliased(1, _s, target);
  }

  // optional uint64 optional_uint64 = 2;
  if ((cached_has_bits & 0x00000004U) != 0) {
    target = stream->EnsureSpace(target);
    target = ::_pbi::WireFormatLite::WriteUInt64ToArray(
        2, this_._internal_optional_uint64(), target);
  }

  // optional float optional_float = 3;
  if ((cached_has_bits & 0x00000008U) != 0) {
    target = stream->EnsureSpace(target);
    target = ::_pbi::WireFormatLite::WriteFloatToArray(
        3, this_._internal_optional_float(), target);
  }

  // .google.protobuf.Any any = 4;
  if ((cached_has_bits & 0x00000002U) != 0) {
    target = ::google::protobuf::internal::WireFormatLite::InternalWriteMessage(
        4, *this_._impl_.any_, this_._impl_.any_->GetCachedSize(), target,
        stream);
  }

  if (ABSL_PREDICT_FALSE(this_._internal_metadata_.have_unknown_fields())) {
    target =
        ::_pbi::WireFormat::InternalSerializeUnknownFieldsToArray(
            this_._internal_metadata_.unknown_fields<::google::protobuf::UnknownFieldSet>(::google::protobuf::UnknownFieldSet::default_instance), target, stream);
  }
  // @@protoc_insertion_point(serialize_to_array_end:libfuzzer_example.Msg)
  return target;
}

#if defined(PROTOBUF_CUSTOM_VTABLE)
::size_t Msg::ByteSizeLong(const MessageLite& base) {
  const Msg& this_ = static_cast<const Msg&>(base);
#else   // PROTOBUF_CUSTOM_VTABLE
::size_t Msg::ByteSizeLong() const {
  const Msg& this_ = *this;
#endif  // PROTOBUF_CUSTOM_VTABLE
  // @@protoc_insertion_point(message_byte_size_start:libfuzzer_example.Msg)
  ::size_t total_size = 0;

  ::uint32_t cached_has_bits = 0;
  // Prevent compiler warnings about cached_has_bits being unused
  (void)cached_has_bits;

  ::_pbi::Prefetch5LinesFrom7Lines(&this_);
  cached_has_bits = this_._impl_._has_bits_[0];
  total_size += static_cast<bool>(0x00000008U & cached_has_bits) * 5;
  if ((cached_has_bits & 0x00000007U) != 0) {
    // optional string optional_string = 1;
    if ((cached_has_bits & 0x00000001U) != 0) {
      total_size += 1 + ::google::protobuf::internal::WireFormatLite::StringSize(
                                      this_._internal_optional_string());
    }
    // .google.protobuf.Any any = 4;
    if ((cached_has_bits & 0x00000002U) != 0) {
      total_size += 1 +
                    ::google::protobuf::internal::WireFormatLite::MessageSize(*this_._impl_.any_);
    }
    // optional uint64 optional_uint64 = 2;
    if ((cached_has_bits & 0x00000004U) != 0) {
      total_size += ::_pbi::WireFormatLite::UInt64SizePlusOne(
          this_._internal_optional_uint64());
    }
  }
  return this_.MaybeComputeUnknownFieldsSize(total_size,
                                             &this_._impl_._cached_size_);
}

void Msg::MergeImpl(::google::protobuf::MessageLite& to_msg, const ::google::protobuf::MessageLite& from_msg) {
  auto* const _this = static_cast<Msg*>(&to_msg);
  auto& from = static_cast<const Msg&>(from_msg);
  if constexpr (::_pbi::DebugHardenVerifyHasBitConsistency()) {
    from.VerifyHasBitConsistency();
  }
  ::google::protobuf::Arena* arena = _this->GetArena();
  // @@protoc_insertion_point(class_specific_merge_from_start:libfuzzer_example.Msg)
  ABSL_DCHECK_NE(&from, _this);
  ::uint32_t cached_has_bits = 0;
  (void) cached_has_bits;

  cached_has_bits = from._impl_._has_bits_[0];
  if ((cached_has_bits & 0x0000000fU) != 0) {
    if ((cached_has_bits & 0x00000001U) != 0) {
      _this->_internal_set_optional_string(from._internal_optional_string());
    }
    if ((cached_has_bits & 0x00000002U) != 0) {
      ABSL_DCHECK(from._impl_.any_ != nullptr);
      if (_this->_impl_.any_ == nullptr) {
        _this->_impl_.any_ = ::google::protobuf::Message::CopyConstruct(arena, *from._impl_.any_);
      } else {
        _this->_impl_.any_->MergeFrom(*from._impl_.any_);
      }
    }
    if ((cached_has_bits & 0x00000004U) != 0) {
      _this->_impl_.optional_uint64_ = from._impl_.optional_uint64_;
    }
    if ((cached_has_bits & 0x00000008U) != 0) {
      _this->_impl_.optional_float_ = from._impl_.optional_float_;
    }
  }
  _this->_impl_._has_bits_[0] |= cached_has_bits;
  _this->_internal_metadata_.MergeFrom<::google::protobuf::UnknownFieldSet>(from._internal_metadata_);
}

void Msg::CopyFrom(const Msg& from) {
// @@protoc_insertion_point(class_specific_copy_from_start:libfuzzer_example.Msg)
  if (&from == this) return;
  Clear();
  MergeFrom(from);
}


void Msg::InternalSwap(Msg* PROTOBUF_RESTRICT PROTOBUF_NONNULL other) {
  using ::std::swap;
  auto* arena = GetArena();
  ABSL_DCHECK_EQ(arena, other->GetArena());
  _internal_metadata_.InternalSwap(&other->_internal_metadata_);
  swap(_impl_._has_bits_[0], other->_impl_._has_bits_[0]);
  ::_pbi::ArenaStringPtr::InternalSwap(&_impl_.optional_string_, &other->_impl_.optional_string_, arena);
  ::google::protobuf::internal::memswap<
      PROTOBUF_FIELD_OFFSET(Msg, _impl_.optional_float_)
      + sizeof(Msg::_impl_.optional_float_)
      - PROTOBUF_FIELD_OFFSET(Msg, _impl_.any_)>(
          reinterpret_cast<char*>(&_impl_.any_),
          reinterpret_cast<char*>(&other->_impl_.any_));
}

::google::protobuf::Metadata Msg::GetMetadata() const {
  return ::google::protobuf::Message::GetMetadataImpl(GetClassData()->full());
}
// @@protoc_insertion_point(namespace_scope)
}  // namespace libfuzzer_example
namespace google {
namespace protobuf {
}  // namespace protobuf
}  // namespace google
// @@protoc_insertion_point(global_scope)
PROTOBUF_ATTRIBUTE_INIT_PRIORITY2 static ::std::false_type
    _static_init2_ [[maybe_unused]] =
        (::_pbi::AddDescriptors(&descriptor_table_test_2eproto),
         ::std::false_type{});
#include "google/protobuf/port_undef.inc"
